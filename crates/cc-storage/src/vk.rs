...
// TODO: запрос не просит область `openid`, поэтому `id_token` провайдер не
// выдаёт и проверять подпись, `iss`, `aud`, `exp` и `nonce` нечего
// (`TODO.md`, раздел 4.3, пункт 5). Понадобится область — понадобится и
// `openidconnect`: TASK-058.
...
impl Vk {
    ...
    ///
    /// Адрес возврата задаётся конфигурацией и от клиента не принимается
    /// вовсе: сверять его с чем-либо не приходится, а увести код авторизации
    /// подменой адреса — не через что (`TODO.md`, раздел 4.3).
    #[must_use]
    pub fn new(
