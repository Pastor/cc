...
impl Version {
    ...
    ///
    /// Самая старая поддерживаемая, а не самая новая: клиент, написанный под
    /// первую версию и не присылающий заголовок, не должен ломаться в день
    /// выпуска второй.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn fallback_is_the_oldest_supported() {
        assert_eq!(
            Version::fallback(),
            *Version::SUPPORTED.iter().min().unwrap(),
            "умолчанием оказалась не самая старая поддерживаемая версия"
        );
    }
}
